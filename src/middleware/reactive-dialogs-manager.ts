...
export default class ReactiveDialogManager {
    ...
    constructor(app) {
        ...
        app.setCapability('urn:io.iopa.bot:reactive-dialogs', {
            ...
            set localResourceProtocolMapper(
    ...
    private _matchBeginFlow(
        ...
        if (!flowId) {
            console.log(
                'No current dialog, and could not find as launch intent'
            )
    ...
    private _continueFlow(context: IopaBotContext, next: () => Promise<void>) {
        ...
        const intentFilters = lastPromptActions.map(
            (action) =>
                action.props.intents ||
                action.props.utterances || [toString(action)]
        )
        ...
        switch (action.props.type) {
            ...
            case 'openurl':
                return this.renderActionOpenUrl(
                    action as ActionOpenUrlElement,
                    context
                )
    ...
    protected register(
        ...
        if (flow.type === 'table') {
            ...
            lists.forEach((list) => {
                ...
                console.log(
                    ` registered table ${tableId}  list ${list.props.id}`
                )
            })
            return
        }
        ...
        if (flow.props.utterances && flow.props.utterances.length > 0) {
            if (!Array.isArray(flow.props.utterances)) {
                throw new Error(
                    'utterances on <flow> must be an array of strings'
                )
            }
            ...
            const existingIntent = launchSkill.lookupIntent(
                flow.props.utterances
            )
            ...
            if (existingIntent) {
                ...
                launchSkill.intent(launchName, {
                    utterances: flow.props.utterances,
                })
    ...
    protected registerDialogStep(dialog: DialogElement, skill: Skill) {
        dialog.props.children
            .filter(
                (dialogChild) =>
                    dialogChild.type === 'card' ||
                    typeof dialogChild.type === 'function'
            )
            .forEach((dialogChild) => {
    ...
    protected registerDialogCard(card: CardElement, skill: Skill) {
        card.props.children
            ...
            .forEach((actionset) => {
                this.registerDialogCardActions(
                    actionset as ActionSetElement,
                    skill
                )
            })
    ...
    protected renderFlowById(
        ...
        if (
            ...
            dialogStep = flow.props.children.find(
                (c) => c.props.id === dialogId
            )
            if (!dialogStep) {
    ...
    protected async renderDialogStep(
        ...
        const isNotWaitingOnPrompt = await asyncForEachIfTrue(
            ...
            async (directive, i) => {
                ...
                console.log(
                    `Performing dialog step ${dialog.props.id} directive ${i}`
                )
    ...
    protected async renderCard(
        ...
        const actionset:
            | ActionSetElement
            | undefined = element.props.children.find(
    ...
    private saveActionsFromCard(
        ...
        const [botSession, setBotSession] = useBotSession(context)
        const currentDialog: SessionCurrentDialog = botSession[
            BOT.CurrentDialog
        ]!
    ...
    protected async renderActionOpenUrl(
        ...
        switch (url.protocol) {
            case 'dialog:':
                ...
                await this.renderActionDialogFlow(
                    flowId,
                    dialogId,
                    element,
                    context
            ...
            case 'https:':
            case 'http:':
                await this.renderActionCommand(
                    'openurl',
                    { url },
                    element,
                    context
    ...
    protected async renderActionDialogFlow(
        ...
        await reactive.renderFlow(id, dialogId, context, () =>
            Promise.resolve()
        )
...
const delay = (context, interval) => {
    return new Promise<void>((resolve) => {
        setTimeout(
            resolve,
            context.response[BOT.isDelayDisabled] ? 40 : interval
        )
    })
}
